...
class EMASmoothing:
    ...
    """
    def __init__(self, alpha=0.3, confidence_threshold=0.8):
        """
        Args:
...
def run_hand_landmarks(model_path, camera_index=0, width=None, height=None):
    ...
    ema_smoothers = {
        "LEFT": {
            "wrist": EMASmoothing(alpha=0.4, confidence_threshold=0.8),
            "thumb": EMASmoothing(alpha=0.4, confidence_threshold=0.8),
            "index": EMASmoothing(alpha=0.4, confidence_threshold=0.8),
            "pinky": EMASmoothing(alpha=0.4, confidence_threshold=0.8),
        ...
        while True:
            ...
            if result.hand_landmarks:
                ...
                for hand_idx, hand in enumerate(result.hand_landmarks):
                    ...
                    confidence_array = np.array([hand_confidence])
                    smoothed_confidence_array = ema_smoothers[hand_label]["confidence"].smooth(confidence_array, confidence=1.0)
                    smoothed_confidence = smoothed_confidence_array[0]
                    ...
                    # Get DISCRETE orientation
                    discrete_orientation = classify_palm_orientation(palm_z)
                    
                    # Print discrete result to console
                    print(f"{hand_label} Hand: {discrete_orientation} | Palm Normal: X={palm_z[0]:.3f}, Y={palm_z[1]:.3f}, Z={palm_z[2]:.3f}")

                    # -------------------------
                    # Compute hand-centered visualization axes (using smoothed keypoints)
                    # -------------------------
                    # Note: wrist, thumb, index_mcp, pinky_mcp are already smoothed from above
                    ...
                    # Display detection confidence (both raw and smoothed)
                    confidence_color = (0, 255, 255) if smoothed_confidence > 0.7 else (0, 165, 255)
                    cv2.putText(annotated,
                                f"Confidence: {hand_confidence:.3f} -> {smoothed_confidence:.3f}",
                                (10, 60 + hand_idx * 120), cv2.FONT_HERSHEY_SIMPLEX, 0.7, confidence_color, 2)
                    
                    # -------------------------
                    # Display both old and new yaw/pitch/roll
                    # -------------------------
                    #cv2.putText(annotated,
                    #            f"YPR: Y:{yaw_new:.1f} P:{pitch_new:.1f} R:{roll_new:.1f}",
                    #            (10, 60 + hand_idx * 100), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255,200,0), 2)
