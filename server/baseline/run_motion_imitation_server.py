...
import numpy as np
from server.common import visualizations
import queue as thread_queue
from server.common import pose_mapping as pose_map
from server.common import websocket_client as clientws
from server.common.angle_calculator import AngleCalculator
from server.common.angle_classifier_pepper import AngleClassifier
from server.common.hand_detection.EMA_smoothing import EMASmoothing
from server.common import hand_orientation as ho

from multiprocessing import Event, Process, Queue
from server.common.perf_metrics import consume_send_metrics, make_send_metrics
from server.common.websocket_client import _put_latest, _encode_pose_base64, shutdown_server
...
def main():
    ...
    try:
        ...
        while not stop_event.is_set():
            ...
            pose_feasibility = {"is_feasible": False, "violations": []}
            pepper_feasibility = {"is_feasible": False, "violations": []}
            ...
            if active_client is not None and pending_pose_transfer is not None:
                ...
                # Compute BOTH human and pepper angles from the SAME full payload being sent to client
                # This also computes forearm directions, upper arm directions, and wristyaw angles
                both_angles = calculator.compute_both_angles_from_payload(full_payload)
                human_angles = both_angles.get("human", {})

                print(f"Human angles: {human_angles}")
                ...
                pose_feasibility = classifier.check_feasibility(
                    human_angles,
                    forearm_direction_right,
                    forearm_direction_left,
                    orientation_labels,
                )
                pepper_feasibility = classifier.check_feasibility(
                    pepper_angles,
                    forearm_direction_right,
                    forearm_direction_left,
                    orientation_labels,
            ...
            if not human_angles and poses3d_vis is not None and poses3d_vis.shape[0] > 0:
                ...
                pose_feasibility = classifier.check_feasibility(human_angles, None, None, None)
                pepper_feasibility = classifier.check_feasibility(pepper_angles, None, None, None)
            
            # Compute singularity checks for both arms (always compute if we have angles)
            if human_angles:
                singularity_right = classifier.check_singularity_poses(human_angles, arm='right', 
                                                                       shoulder_roll_threshold=30, 
                                                                       elbow_roll_threshold=20.0)
                print(singularity_right)
                singularity_left = classifier.check_singularity_poses(human_angles, arm='left', 
                                                                      shoulder_roll_threshold=30, 
                                                                      elbow_roll_threshold=20.0)
