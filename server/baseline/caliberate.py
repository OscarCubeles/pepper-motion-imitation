...
def main():
    ...
    objp = np.zeros(
        (settings.CHECKERBOARD[0] * settings.CHECKERBOARD[1], 3),
        np.float32,
    )
    objp[:, :2] = np.mgrid[
        0:settings.CHECKERBOARD[0], 0:settings.CHECKERBOARD[1]
    ].T.reshape(-1, 2)
    ...
    if not ret:
        cap.release()
        raise RuntimeError("Could not read an initial webcam frame")
    h, w = frame.shape[:2]

    diag_px = math.sqrt(w**2 + h**2)
    f_init = diag_px / (
        2 * math.tan(math.radians(settings.DIAGONAL_FOV_DEG) / 2)
    )
    ...
    while True:
        ...
        if found:
            ...
            cv2.drawChessboardCorners(
                vis, settings.CHECKERBOARD, corners, found
            )
    ...
    ret, intrinsic_matrix, dist, _rvecs, _tvecs = cv2.calibrateCamera(
        objpoints,
        imgpoints,
        (w, h),
        k_init,
    ...
    fov_diag = 2 * math.degrees(
        math.atan(math.sqrt(w**2 + h**2) / (2 * fx))
    )
