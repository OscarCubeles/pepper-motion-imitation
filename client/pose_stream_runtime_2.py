...
def _decode_dispatch_loop():
    while not stop_event.is_set():
        ...
        if message is None:
            break
        try:
            # Try to parse as JSON (new format with hand orientation and angles)
            payload_bytes = None
            frame_meta = None
            ...
            try:
                ...
                if isinstance(json_data, dict) and "pose_keypoints" in json_data:
                    # New JSON format with hand orientation labels and angles
                    pose_base64 = json_data.get("pose_keypoints")
                    hand_orientation = json_data.get("hand_orientation", {})
                    angles_data = json_data.get("angles", None)  # Extract angles from payload
                    if angles_data:
                        print("[PAYLOAD] Angles extracted from server payload: Available")
            ...
            recieve_joints(
                ...
                angles=angles_data,  # Pass angles to recieve_joints
            )
        except Exception as exc:
...
def recieve_joints(jointMatrix, frame_num=0, frame_id=None, capture_ts_client_us=None, hand_orientation=None, angles=None):
    ...
    if not np.all(jointMatrix == 0.0):
        ...
        if hand_orientation:
            print("[JOINTS] Hand Orientation: {}".format(hand_orientation))
        
        # Print angles received from server
        if angles is not None:
            pepper_angles_temp = angles.get("pepper", {})
            print("[ANGLES RECEIVED] R_ShoulderRoll={:.4f} R_WristYaw={:.4f} L_ShoulderRoll={:.4f} L_WristYaw={:.4f}".format(
                pepper_angles_temp.get("ShoulderRoll_Right", 0),
                pepper_angles_temp.get("RWristYaw", 0),
                pepper_angles_temp.get("ShoulderRoll_Left", 0),
                pepper_angles_temp.get("LWristYaw", 0)
            ))
        
        # If angles are available from server, use them directly for Pepper commands
        if angles is not None and ENABLE_ROBOT_IMITATION:
            pepper_angles = angles.get("pepper", {})  # Angles in radians from server
            print("[ANGLES RECEIVED] Pepper angles (radians): {}".format(pepper_angles))
            if pepper_angles:
                # Use server-computed angles directly
                names = []
                angles_list = []
                speeds = []
                
                # Extract speeds from server payload (defaults to 0.5 if not available)
                speeds_data = angles.get("speeds", {})
                
                # Map angle names and values - use correct NAOqi joint names
                angle_mapping = [
                ...
                for angle_key, naoqi_name in angle_mapping:
                    if angle_key in pepper_angles and pepper_angles[angle_key] is not None:
                        angle_val = pepper_angles[angle_key]
                        # Angles from server are in radians, Pepper expects radians
                        names.append(naoqi_name)
                        angles_list.append(angle_val)  # Use radians directly
                        # Use speed from server, default to 0.5 if not available
                        speed = speeds_data.get(angle_key, 0.5)
                        speeds.append(speed)
                
                if angles_list:
                    print("[COMMAND] Joint targets being sent to Pepper:")
                    for name, angle, speed in zip(names, angles_list, speeds):
                        print("  - {}: {:.4f} rad ({:.2f}°) @ {:.2f} speed".format(
                            name, angle, np.degrees(angle) if angle is not None else 0, speed))
                    ...
                    return  # Skip the rest of the pose-based computation
        print("None of the angles data from the server was usable for Pepper commands, falling back to pose-based IK.")
        # Head
        Nose = jointMatrix[0]
        Neck = jointMatrix[1]
        # Right arm
        ...
        # If we get here, use pose-based computation (backward compatibility)
        if ENABLE_ROBOT_IMITATION:
