...
# Function that returns the hand value given the position of the thumb, the tip of the hand and the previous distance between them
def hand_open_close_right(thumb, tip, prev_dist):
    # multiply by 10 to get values from 0 to 1
    dis = sqrt((thumb[0]-tip[0])**2 + (thumb[1]-tip[1])**2 + (thumb[2]-tip[2])**2) * 10
    
    if dis <= 0.4:
        dis = 0
    # dis = abs(sqrt((thumb[0]-tip[0])**2 + (thumb[1]-tip[1])**2 + (thumb[2]-tip[2])**2) * 10 - 0.3)
    # print("right: " + str(dis))
    if abs(prev_dist-dis) < 0.2:
        dis = prev_dist
...
def hand_open_close_left(thumb, tip, prev_dist):
# multiply by 10 to get values from 0 to 1
    dis = sqrt((thumb[0]-tip[0])**2 + (thumb[1]-tip[1])**2 + (thumb[2]-tip[2])**2) * 10
...
def get_torso_angles_constrained(px, py, pz):
    global prev_hipPitch_const
    l3 = 0.02
    l4 = 139.0
    ...
    if term1 > 1.0:
        # hipPitch = 0.0
        hipPitch = prev_hipPitch_const
        kneePitch = pairs[hipPitch]
        # print("I'm a term > 1.0")
    else:
        t2_1 = arcsin(term1) - arctan2(n,l3)
        t2_2 = pi - arcsin(term1) - arctan2(n,l3)

        if t2_1.imag != 0 or t2_1.real < -1.0385 or t2_1.real > 1.0385:
            t2_1 = nan

        if t2_2.imag != 0 or t2_2.real < -1.0385 or t2_2.real > 1.0385:
            t2_2 = nan
        ...
        if isnan(dist1) and isnan(dist2):
            ...
            hipPitch = prev_hipPitch_const
            kneePitch = pairs[hipPitch]
            # print("dist1 and dist2 are nan; prev HipPitch = " + str(prev_hipPitch_const))
        elif dist1 < dist2 or isnan(dist2):
            # constraining t2 to be smaller than actually
            if t2_1 < -0.8:
                t2_1 = -0.8
            elif t2_1 > 0.5:
                t2_1 = 0.5
    ...
    prev_hipPitch_const = round(hipPitch, 2)
